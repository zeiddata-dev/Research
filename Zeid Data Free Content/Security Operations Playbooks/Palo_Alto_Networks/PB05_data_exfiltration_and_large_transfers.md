# Zeid Data Security Playbooks — Palo Alto Networks — Data Exfiltration and Large Transfers

**Authorized SOC use only. Use only on systems/data you own or have explicit permission to analyze.**

**Playbook ID:** PAL-PB05  
...
Investigate potential data exfil via large downloads, uploads, or unusual egress.

## Trigger and severity

**Triggers (examples):**
- SIEM alert/correlation search fires for **data exfiltration and large transfers**
...
**Vendor note:** Palo Alto playbooks focus on threat/URL/traffic analytics and remote access patterns.
...
**Palo Alto pivots:**
- URL categories, TLS SNI/metadata, rare destinations, correlating GlobalProtect logins.

**Splunk (example):**

```spl
index=pan* sourcetype in (pan:traffic, pan:threat, pan:url, pan:globalprotect) (bytes_out OR sent_bytes OR download OR upload)
| stats sum(bytes_out) as total_out by user, src_ip, dest
...
```kusto
CommonSecurityLog
| summarize bytesOut=sum(tolong(SentBytes)) by SourceIP, DestinationIP, bin(TimeGenerated, 1h)
| where bytesOut > 1000000000
```
