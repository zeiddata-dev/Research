...
Investigate movement across hosts/accounts (RDP/SMB/WinRM/API pivoting).

## Trigger and severity

**Triggers (examples):**
- SIEM alert/correlation search fires for **lateral movement**
...
```spl
index=pan* sourcetype in (pan:traffic, pan:threat, pan:url, pan:globalprotect) (EventCode=4624 OR logon OR rdp OR smb OR winrm)
| stats count values(LogonType) as types values(src_ip) as ips by user, dest
| where count > 50
```
...
| summarize logons=count(), hosts=make_set(Computer) by Account, bin(TimeGenerated, 30m)
| where logons > 50
```
