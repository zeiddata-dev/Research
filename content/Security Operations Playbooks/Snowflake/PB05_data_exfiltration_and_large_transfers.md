# Zeid Data Security Playbooks — Snowflake — Data Exfiltration and Large Transfers

**Authorized SOC use only. Use only on systems/data you own or have explicit permission to analyze.**

**Playbook ID:** SNO-PB05  
**Vendor:** Snowflake  
...
**Snowflake pivots (ACCOUNT_USAGE / exported logs):**
- Look for `COPY INTO` to external stages, `GET/PUT`, unusually large query results.
- Confirm role, warehouse, client app name, and network policies.

**Splunk (example):**

```spl
index=snowflake* sourcetype=snowflake:* (bytes_out OR sent_bytes OR download OR upload)
