# Zeid Data Security Playbooks — Google Workspace — Insider Risk and Sensitive Access

**Authorized SOC use only. Use only on systems/data you own or have explicit permission to analyze.**

**Playbook ID:** GOO-PB09  
...
Investigate anomalous access to sensitive resources by a legitimate user.

## Trigger and severity

**Triggers (examples):**
- SIEM alert/correlation search fires for **insider risk and sensitive access**
...
**Google Workspace pivots:**
- Drive: mass downloads/shares, permission changes, external share spikes.
- Gmail: forwarding rules, delegate access changes.

**Splunk (example):**

```spl
index=google_workspace* sourcetype in (gws:login, gws:admin, gws:drive, gws:gmail) (sensitive OR confidential OR pii OR export)
| stats count values(dest) as targets by user
```
...
```kusto
OfficeActivity
| where Operation has_any ("FileDownloaded","FileSyncDownloadedFull","MailItemsAccessed")
| summarize ops=count() by UserId, bin(TimeGenerated, 1h)
| where ops > 500
```
