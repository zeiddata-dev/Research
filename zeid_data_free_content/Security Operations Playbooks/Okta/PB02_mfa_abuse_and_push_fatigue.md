...
Detect and respond to MFA bombing/push fatigue and suspicious MFA resets.

## Trigger and severity

**Triggers (examples):**
- SIEM alert/correlation search fires for **mfa abuse and push fatigue**
...
**Vendor note:** Okta playbooks focus on auth anomalies, MFA resets, app grants, and admin actions.
...
**Okta pivots:**
- Factor enrollment changes, MFA resets, repeated sign-in challenges, policy edits.

**Splunk (example):**

```spl
index=okta* sourcetype=okta:system (mfa OR "push" OR "factor")
...
```kusto
AuditLogs
| where OperationName has_any ("Update user", "Reset", "MFA", "Authentication")
| summarize events=count() by tostring(InitiatedBy.user.userPrincipalName), bin(TimeGenerated, 15m)
| where events > 10
```
