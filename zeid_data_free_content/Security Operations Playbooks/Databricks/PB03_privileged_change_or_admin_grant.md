...
**Databricks pivots (workspace audit logs):**
- Permission edits, admin role changes, token creation, cluster policy updates.

**Splunk (example):**

```spl
index=databricks* sourcetype=databricks:audit (admin OR role OR privilege OR "add member" OR "grant")
