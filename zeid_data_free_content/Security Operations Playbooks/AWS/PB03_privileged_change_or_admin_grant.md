...
**AWS pivots (CloudTrail/GuardDuty exports):**
- Review `ConsoleLogin`, `AssumeRole`, `CreateAccessKey`, `AttachUserPolicy`, `PutUserPolicy` events.
- Confirm MFA context and source IP consistency where available.

**Splunk (example):**

```spl
index=aws* sourcetype in (aws:cloudtrail, aws:guardduty, aws:securityhub, aws:vpcflow) (admin OR role OR privilege OR "add member" OR "grant")
